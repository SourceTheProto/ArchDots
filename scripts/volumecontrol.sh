...
notify_mute() {
    ...
    if [ "${mute}" == "true" ]; then
        echo "notify-send cancelled" #notify-send -a "t2" -r 91190 -t 800 -i "${icodir}/muted-${dvce}.svg" "muted" "${nsink}"
    else
        echo "notify-send cancelled" #notify-send -a "t2" -r 91190 -t 800 -i "${icodir}/unmuted-${dvce}.svg" "unmuted" "${nsink}"
    fi
}
